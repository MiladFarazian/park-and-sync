...
import { MapPin, Clock, Shield, Search, Calendar, ChevronRight, Loader2, Navigation } from 'lucide-react';
...
const HeroSection = () => {
  ...
  const reverseGeocode = async (lat: number, lng: number) => {
    ...
    try {
      const response = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${lng},${lat}.json?access_token=${mapboxToken}&types=address,neighborhood,place`);
  ...
  return <section className="relative min-h-[calc(100vh-64px)] flex items-center bg-background">
      <div className="container mx-auto px-6 py-12 lg:py-20">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          {/* Left: Content */}
          <div className="space-y-8 lg:space-y-10">
            <div className="space-y-4">
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight tracking-tight">Parking, 
for the people<span className="block text-foreground">wherever you go</span>
              </h1>
            ...
            <div className="bg-card border rounded-2xl p-6 space-y-4 shadow-lg">
              ...
              <div className="relative">
                ...
                <Input value={searchLocation} onChange={handleSearchChange} onFocus={() => setShowSuggestions(true)} onBlur={() => {
                setTimeout(() => setShowSuggestions(false), 200);
              }} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder={isLoadingLocation ? "Detecting your location..." : "Where are you going?"} className="pl-12 pr-12 h-14 text-base border-muted bg-muted/30 rounded-xl" />
                {isLoadingLocation ? <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground animate-spin" /> : <button onClick={handleUseCurrentLocation} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-primary transition-colors" title="Use current location">
                    <Navigation className="h-5 w-5" />
                  </button>}
                
                {/* Suggestions Dropdown */}
                {showSuggestions && <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-background border border-border rounded-lg shadow-lg max-h-60 overflow-y-auto">
                  ...
                  setShowSuggestions(false);
                    ...
                    {/* Search Suggestions */}
                    {suggestions.map((suggestion, index) => <button key={suggestion.mapbox_id || index} onMouseDown={e => {
                  e.preventDefault();
                  handleSelectLocation(suggestion);
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            {suggestion.place_formatted && <p className="text-xs text-muted-foreground truncate">{suggestion.place_formatted}</p>}
                          </div>
                        </div>
                      </button>)}
                  </div>}
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setMobileStartPickerOpen(true)} className="flex items-center gap-3 p-4 rounded-xl border border-muted bg-muted/30 hover:bg-muted/50 transition-colors text-left">
                ...
                <button onClick={() => setMobileEndPickerOpen(true)} className="flex items-center gap-3 p-4 rounded-xl border border-muted bg-muted/30 hover:bg-muted/50 transition-colors text-left">
            ...
            {/* List Your Spot CTA */}
            <button onClick={() => navigate('/list-spot')} className="flex items-center gap-2 text-primary hover:text-primary/80 transition-colors font-medium">
          ...
          <div className="relative hidden lg:block">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl">
              <img src={heroImage} alt="Modern parking spot in urban setting" className="w-full h-[560px] object-cover" />
      ...
      {/* Mobile Time Pickers */}
      {mobileStartPickerOpen && <MobileTimePicker isOpen={mobileStartPickerOpen} onClose={() => setMobileStartPickerOpen(false)} onConfirm={date => {
      handleStartTimeChange(date);
      setMobileStartPickerOpen(false);
    }} mode="start" initialValue={startTime} />}

      {mobileEndPickerOpen && <MobileTimePicker isOpen={mobileEndPickerOpen} onClose={() => setMobileEndPickerOpen(false)} onConfirm={date => {
      handleEndTimeChange(date);
      setMobileEndPickerOpen(false);
    }} mode="end" startTime={startTime} initialValue={endTime} />}
    </section>;
};
