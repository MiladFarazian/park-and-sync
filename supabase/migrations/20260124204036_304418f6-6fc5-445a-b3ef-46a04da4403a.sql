-- Fix create_booking_hold_atomic to use the actual booking_status enum values
...
BEGIN
  -- Optional idempotency: return existing active hold for this user + key
  ...
  -- Conflicting bookings: only statuses that represent an in-flight or active booking
  ...
  -- Conflicting holds (active + not expired), exclude same user
  SELECT COUNT(*) INTO v_conflict_count
  FROM booking_holds
  WHERE spot_id = p_spot_id
    AND status = 'active'
  ...
  -- Expire any overlapping holds by the same user
