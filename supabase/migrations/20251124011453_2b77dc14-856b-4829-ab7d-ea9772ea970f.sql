...
-- Add parameter to exclude a specific booking from availability check
...
BEGIN
  ...
  -- Check for conflicting bookings (excluding the current booking if provided)
  IF EXISTS (
...
-- Update validate_booking trigger to exclude current booking during UPDATE
...
BEGIN
  ...
  -- Check spot availability, excluding:
  -- 1. The renter's own holds (via p_exclude_user_id)
  -- 2. The current booking itself during updates (via p_exclude_booking_id)
