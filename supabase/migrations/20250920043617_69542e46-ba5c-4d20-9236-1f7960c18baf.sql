...
-- Create unique constraint to prevent overlapping bookings
...
-- Create unique constraint to prevent overlapping holds
ALTER TABLE public.booking_holds ADD CONSTRAINT no_overlapping_holds
EXCLUDE USING GIST (
  spot_id WITH =,
  tsrange(start_at, end_at, '[)') WITH &&
...
BEGIN
  ...
  IF EXISTS (
    ...
    AND tsrange(start_at, end_at, '[)') && tsrange(p_start_at, p_end_at, '[)')
  ) THEN
    RETURN FALSE;
  END IF;
